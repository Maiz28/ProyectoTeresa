...
@Component({
  ...
  styleUrls: ['./registro.component.scss']//componentes de los que va a depender
})
export class RegistroComponent implements OnInit {//clase principal

  signupForm!: FormGroup; //hace referencia a nuestro fomulario de registro del html (se hizo en form)
  hidePwd = true;
  hideConfirmPwd = true;
  
  constructor(private authService: AuthService, private router: Router) {}

  ngOnInit(): void {//esta funcion inicializa el form para que le diga que creara un form para luego enviarlo
  ...
  createFormGroup(): FormGroup {
    ...
    return new FormGroup({
      //estos son los campos del formularioo
  ...
  //si un campo esta vasio que valide
  ...
  signup(): void { //será de tipo vacio
    this.authService.signup(this.signupForm.value).subscribe((msg) => {
      ...
      this.router.navigate(["login"]);//lleve al login
      alert("usuario registrado con exito :3");
